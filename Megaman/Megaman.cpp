...
Megaman::Megaman() :CMoveableObject()
{
	_typeID = ID_ROCKMAN;
}
...
void Megaman::Render(CTimer* gameTime, MGraphic* graphics)
{
	if ((_isRequireStopScreen ^ _behave != Behave::DYING) && _behave != Behave::REAL_DIE)
	{
			...
			switch (_behave)
		{
			...
			case STAND_FIRE:
				if (_isRight)
					graphics->Draw(_sprite.GetTexture(), _sprite.GetDestinationRectangle(), _position + D3DXVECTOR2(5, 0), true);
				else
					graphics->Draw(_sprite.GetTexture(), _sprite.GetDestinationRectangle(), _position - D3DXVECTOR2(5, 0), true, SpriteEffects::FLIP_HORIZONTALLY);
				break;
			case STAIR_FIRE:
				if (_isRight)
					graphics->Draw(_sprite.GetTexture(), _sprite.GetDestinationRectangle(), _position + D3DXVECTOR2(5, 0), true);
		...
		{
			graphics->Draw(_confuses[0].GetTexture(), _confuses[0].GetDestinationRectangle(), _position + D3DXVECTOR2(-GetBox()._width / 2 - 1, GetBox()._height / 2 + 3), true);
			graphics->Draw(_confuses[1].GetTexture(), _confuses[1].GetDestinationRectangle(), _position + D3DXVECTOR2(0, GetBox()._height / 2 + 5), true);
			graphics->Draw(_confuses[2].GetTexture(), _confuses[2].GetDestinationRectangle(), _position + D3DXVECTOR2(GetBox()._width / 2 + 1, GetBox()._height / 2 + 3), true);
...
#pragma region Xử lý tiền va chạm, các đối tượng được dự đoán là va chạm trong frame trước được xử lý tương ứng ở đây
		...
		for (int i = 0; i < collidedLst.size(); i++)
		{
			if (collidedLst[i]->_direction == CDirection::INSIDE)
			{
				...
				case  ID_ITEM_LIFE:
					_life += 1;
					((CItemEatable*)collidedLst[i]->_object)->GotIt();
					break;
				...
				default:
					if (_behave != Behave::HURT_IN_AIR&&_behave != Behave::HURT_ON_GROUND&& !_isInShield)
					{
						...
						_isRight = false;
						_isInShield = false;
						_actionPosition = D3DXVECTOR2(_box._x + _box._width / 2, _box._y + 8);
...
void Megaman::SetPos(D3DXVECTOR2 _pos)
{
	this->_position=_pos;
...
void Megaman::ChangeScreen(CDirection direction)
{
	switch (direction)
	{
	case ON_UP:
...
void Megaman::Fire()
{
	...
	case STAND_FIRE:
		...
		case NORMAL:
			if (_isRight)
				_bullets.push_back(new CBulletRockman(D3DXVECTOR2(2 * ROCKMAN_VERLOCITY_X, 0), _position + D3DXVECTOR2(12, 0)));
			else
				_bullets.push_back(new CBulletRockman(D3DXVECTOR2(-2 * ROCKMAN_VERLOCITY_X, 0), _position - D3DXVECTOR2(12, 0)));
...
vector<CBullet*> Megaman::GetBullets()
{
...
void Megaman::ThroughOverDoor(int doorID){
	switch (doorID)
	{
	case ID_DOOR1_CUTMAN:
	case ID_DOOR1_GUTSMAN:
...
bool Megaman::IsGoingOverDoor(){
	return _isGoingOverDoor;
}

bool Megaman::IsOverDoor(){
	return _isOverDoor;
}

bool Megaman::IsRequireOpenDoor()
{
	return _isRequireOverDoor;
}

bool Megaman::IsRequireStopScreen()
{
	return _isRequireStopScreen;
}

map<Skill, int> Megaman::GetSkillInfo()
{
	return _skillInfos;
}

int Megaman::GetWeapons(Skill skill)
{
	return _skillInfos.find(skill)->second;
}

void Megaman::SetWeapons(Skill skill, int weapons)
{
...
Skill Megaman::GetCurrentSkill()
{
	return _currentSkill;
}

void Megaman::ReSetSKill()
{
	_canFire = true;
	_behave = Behave::START;
	_v.x = _v.y = _a.x = _a.y = 0.0f;
