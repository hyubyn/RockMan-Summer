...
CRockCutBullet::CRockCutBullet(Megaman* master) : CBullet(0, ID_BULLET_ROCKMAN_CUT, ResourceManager::GetSprite(ID_SPRITE_BULLET_CUT), 10, D3DXVECTOR2(0, 0), master->_position)
{
	_master = master;
	...
	_timeReturn = 0;
	_timeDelay = 0;
	_defaultVelocity = D3DXVECTOR2(130.0f / 1000.0f, 65.0f / 1000.0f);
	_defaultAccelerate = D3DXVECTOR2(0.075f / 1000.0f, 0.15f / 1000.0f);
...
void CRockCutBullet::Update(CTimer* gameTime)
{
	float collideTime;
	CDirection normalX, normalY;
	collideTime = CheckCollision(this, _master->GetBox(), normalX, normalY, gameTime->GetTime());
	...
	else
	{
		...
		collideTime = CheckCollision(this, Box(_oldPosition.x - 10.0f, _oldPosition.y + 10.0f, 20.0f, 20.0f, 0.0f, 0.0f), normalX, normalY, gameTime->GetTime());
		if (collideTime<gameTime->GetTime())
		{
			_timeReturn = 0;
			Find();
...
void CRockCutBullet::Render(CTimer* gameTime, MGraphic* graphics)
{
...
float CRockCutBullet::CheckCollision(CGameObject* obj1, Box staticBox, CDirection &normalX, CDirection &normalY, float frameTime)
{
