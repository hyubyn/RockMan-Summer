...
CItemObject::CItemObject(int typeID, D3DXVECTOR2 position, bool itemOfBoss, bool isDefault) :CMoveableObject()
{
	_typeID = typeID;
...
void CItemObject::Render(CTimer* gameTime, MGraphic* graphics)
{
	if (!_isGot)
	{
...
void CItemObject::Update(CTimer* gameTime)
{
	if (!_isDefault)
	{
...
void CItemObject::UpdateCollision(CGameObject* obj, CDirection normalX, CDirection normalY, float collideTime, float remainCollideTime)
{
...
void CItemObject::GotIt()
{
	_isGot = true;
}

bool CItemObject::IsGot()
{
	return _isGot;
}

CItemObject* CItemObject::ToValue()
{
