...
CCutManBullet::CCutManBullet(int id, int typeID, CSprite sprite, int dame, D3DXVECTOR2 v, D3DXVECTOR2 beginPosition) :CBullet(id, typeID, sprite, dame, v, beginPosition)
{
	this->_posRockMan = D3DXVECTOR2(0, 0);
...
void CCutManBullet::Update(CTimer* gameTime)
{
...
void CCutManBullet::Fire(double d)
{
	...
	if(!this->_isReverse)
	{
		u = D3DXVECTOR2(this->_posCutMan.x - this->_posRockMan.x, this->_posCutMan.y - this->_posRockMan.y);
		this->_v.x = 55.0f;

	}
	else
	{
		u = D3DXVECTOR2(this->_posCutMan.x - this->_position.x, this->_posCutMan.y - this->_position.y);
		this->_v.x += 50.0f;	
...
void CCutManBullet::Render(CTimer* gameTime, MGraphic* graphics)
{
	if (this->_isHide)
	{
		if (this->_v.x > 0)
