...
CEnemyWorker::CEnemyWorker(int id, int typeID, CSprite sprite, CSprite spriteExplodingEffect, float vX, D3DXVECTOR2 positionBegin, int dame, int blood, int score) :CEnemy()
{
	_id = id;
	_typeID = typeID;
	_sprite = sprite;
	_v = D3DXVECTOR2(vX,-vX);
	_dame = dame;
...
void CEnemyWorker::Update(CTimer* gameTime, Megaman* rockman)
{
	...
	{
		_isAppeared = true;
		if (rockman->GetPos().x >= this->_positionAttack.x)
		{
			_position = D3DXVECTOR2(_positionAttack.x - 50, _positionAttack.y + 50);
	...
	#pragma region Vùng tấn công và phòng thủ
	{
		...
		if (_state == ENEMYWORKER_STATE::ATTACK)
		{
			...
			if (_timeDelayFireRemain <= 0)
			{
				...
				CBullet*  bullet = new CEnemyWorkerBullet(0, ID_BULLET_WORKER,
					...
					DAME_BULLET_WORKER, D3DXVECTOR2(posX, posY),
					abs(rockman->GetPos().x-posX)*4.7,
		...
		_timeAttackOrDefenseRemain -= gameTime->GetTime();
	}
	#pragma endregion
...
void CEnemyWorker::Render(CTimer* gameTime, MGraphic* graphics)
{
