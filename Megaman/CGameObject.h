...
{
private:
	Box _collideRegion;	// vùng va chạm, dùng để kiêm tra đối tượng và lấy ra xét va chạm trên màn hình
public:
	CGameObject();
	virtual  ~CGameObject();
	...
	virtual void Render(CTimer* gameTime, MGraphic* graphics) override;

	virtual CGameObject* ToValue();

	Box GetCollideRegion();
