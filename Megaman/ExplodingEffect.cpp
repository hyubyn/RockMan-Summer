...
CExplodingEffect::CExplodingEffect(D3DXVECTOR2 position, CSprite sprite, int idSoundExplodingEffect) : CGameObject()
{
	_sprite = sprite;
	_position = position;
...
void CExplodingEffect::Render(CTimer* gameTime, MGraphic* graphics)
{
	graphics->Draw(_sprite.GetTexture(), _sprite.GetDestinationRectangle(), _position, true);
}

void CExplodingEffect::Update(CTimer* gameTime)
{
