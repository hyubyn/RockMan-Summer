...
CEnemyBoomBullet::CEnemyBoomBullet(int id, int typeID, CSprite spriteBullet, CSprite spriteExplodingEffect, int dame, D3DXVECTOR2 beginPosition, float lMaxJump, float angleJump)
	: CBullet(id, typeID, spriteBullet, dame, 
	D3DXVECTOR2((sqrt(lMaxJump * GRAVITY / abs(sinf(2 * angleJump)))*cosf(angleJump)), sqrt(lMaxJump * GRAVITY / abs(sinf(2 * angleJump)))*abs(sinf(angleJump)) * 2), //Nhân _v.y cho 2 là để phía dưới trừ _v.y thật nhiều để tạo cảm giác di chuyển nhanh
			beginPosition)
{
...
void CEnemyBoomBullet::Update(CTimer* gameTime)
{
...
#pragma endregion
	...
	if (_state == BULLET_BASE_STATE::FLYING)
	{
		...
		_v.y -= GRAVITY * gameTime->GetTime() * 20;
	}
...
void CEnemyBoomBullet::Render(CTimer* gameTime, MGraphic* graphics)
{
	if (_state == BULLET_BASE_STATE::FLYING)
