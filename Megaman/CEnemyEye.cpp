...
int CEnemyEye::Initlize()
{
	...
	_v = D3DXVECTOR2(100.0f / 1000.0f, 100.0f / 1000.0f);
	UpdateBox();
	return 1;
}
void CEnemyEye::Render(CTimer* gameTime, MGraphic* graphics)
{
	graphics->Draw(_sprite.GetTexture(), _sprite.GetDestinationRectangle(), _position, true);
	CMoveableObject::Render(gameTime, graphics);
}
void CEnemyEye::Update(CTimer* gameTime)
{
...
void CEnemyEye::Update(CTimer* gameTime, Megaman* rockman)
{
