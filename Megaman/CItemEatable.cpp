#include "CItemEatable.h"
CItemEatable::CItemEatable(int typeID, D3DXVECTOR2 position, bool itemOfBoss, bool isDefault) :CMoveableObject()
{
	_typeID = typeID;
...
CItemEatable::~CItemEatable()
{

}

int CItemEatable::Initlize()
{
	_isGot = false;
	_timeAppear = 5000;
...
void CItemEatable::Render(CTimer* gameTime, MGraphic* graphics)
{
	if (!_isGot)
	{
...
void CItemEatable::Update(CTimer* gameTime)
{
	if (!_isDefault)
	{
...
void CItemEatable::UpdateCollision(CGameObject* obj, CDirection normalX, CDirection normalY, float collideTime, float remainCollideTime)
{
...
void CItemEatable::GotIt()
{
	_isGot = true;
}

bool CItemEatable::IsGot()
{
	return _isGot;
}

CItemEatable* CItemEatable::ToValue()
{
	return new CItemEatable(*this);
}