...
CBulletSnapper::CBulletSnapper(int id, int typeID, CSprite spriteBullet, int dame, float v0, D3DXVECTOR2 beginPosition, float angleFly, int x, int y,int xRockMan) :CBullet(id, typeID, spriteBullet, dame, D3DXVECTOR2(v0*cosf(angleFly), v0*sinf(angleFly)), beginPosition)
{
	_position.x = x;
	_position.y = y;
...
void CBulletSnapper::Update(CTimer* gameTime)
{
	if (!_setValueCut)
		SetValueCut();
	else
...
void CBulletSnapper::Render(CTimer* gameTime, MGraphic* graphics)
{
	if (_v.x<0)
	graphics->Draw(_sprite.GetTexture(), _sprite.GetDestinationRectangle(), _position, true);
	else
	graphics->Draw(_sprite.GetTexture(), _sprite.GetDestinationRectangle(), _position, true,D3DXVECTOR2(1,1),SpriteEffects::FLIP_HORIZONTALLY);
}
