...
CBoomManBullet::CBoomManBullet(int id, int typeID, CSprite sprite, int dame, D3DXVECTOR2 v, D3DXVECTOR2 beginPosition) : CBullet(id, typeID, sprite, dame, v, beginPosition)
{
	...
	this->_v = D3DXVECTOR2(28.0f / 1000.0f, 30.0f / 1000.0f);
	this->_posRockManStand = D3DXVECTOR2(0, 0);
...
void CBoomManBullet::Update(CTimer* gameTime)
{
	// boom man die thì reset lại thuộc tính
	if(this->_isBoomManDie)
	{
		this->_idSprite = ID_SPRITE_BULLET_BOSS_BOOM;
		this->_position = D3DXVECTOR2(this->_posBoomMan.x - 8.0f, this->_posBoomMan.y + 18.5f);
		this->_sttBoom = STTBOOM::START;
		this->_isBoomManDie = false;
		this->_timerPush = 0.0f;
		this->_v = D3DXVECTOR2(28.0f / 1000.0f, 60.0f / 1000.0f);
		this->_timeExplosive = TIME_EXPLOSIVE;
	}
	...
	{
		this->_dieDistance += (0.05f)*gameTime->GetTime();
	}
	else //boom đứng yên
	{
...
void CBoomManBullet::Render(CTimer* gameTime, MGraphic* graphics)
{
	...
	case STTBOOM::EXPLOSIVE:
		{
			...
			for (int i = 1; i < 7; i++)
			{
				graphics->Draw(this->_sprite.GetTexture(), this->_sprite.GetDestinationRectangle(), D3DXVECTOR2(this->_dieDistance* cos(i*PI / 3.0f) + this->_position.x, this->_dieDistance *sin(i*PI / 3.0f) + this->_position.y), D3DXVECTOR2(1.0f, 1.0f));
				graphics->Draw(this->_sprite.GetTexture(), this->_sprite.GetDestinationRectangle(), D3DXVECTOR2((this->_dieDistance + 5.0f)* cos(i*PI / 3.0f) + this->_position.x, (this->_dieDistance + 2.0f) *sin(i*PI / 3.0f) + this->_position.y), D3DXVECTOR2(1.0f, 1.0f));
			}
			break;
		}
	default:
