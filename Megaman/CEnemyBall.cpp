...
CEnemyBall::CEnemyBall(int id, int typeID, CSprite sprite, CSprite spriteExplodingEffect, float vX, D3DXVECTOR2 positionBegin, int dame, int blood, int score) :CEnemy()
{
	_id = id;
	_typeID = typeID;
	_sprite = sprite;
...
void CEnemyBall::Update(CTimer* gameTime, Megaman* rockman)
{
	...
	{
		_timeDefenseRemain -= gameTime->GetTime();
		_position.x += gameTime->GetTime()*-abs(_v.x);
	}
...
void CEnemyBall::Render(CTimer* gameTime, MGraphic* graphics)
{
...
void CEnemyBall::AttackRockman(float deltaTime)
{
	...
	for (int i = 0; i < 8; i++)
	{
		CBullet*  bullet = new CEnemyMachineAutoBullet(0, ID_BULLET_BALL,
			...
			DAME_BULLET_BALL, 230.0f / 1000.0f, D3DXVECTOR2(posX, posY),
			angleFly + i * PI / 4.0f);
		bullet->Initlize();
