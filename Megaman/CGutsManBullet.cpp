...
CGutsManBullet::CGutsManBullet(int id, int typeID, CSprite sprite, int dame, D3DXVECTOR2 v, D3DXVECTOR2 beginPosition) : CBullet(id, typeID, sprite, dame, v, beginPosition)
{
...
void CGutsManBullet::Update(CTimer* gameTime)
{
	...
	{
		this->_dieDistance += (0.09f)*gameTime->GetTime();
	}
	else //boom đứng yên
	{
...
void CGutsManBullet::Render(CTimer* gameTime, MGraphic* graphics)
{
	...
	case STTBOOM::EXPLOSIVE:
		{	
			for(int i = -1; i < 2; i++)
			{
				...
				{
					graphics->Draw(this->_sprite.GetTexture(), this->_sprite.GetDestinationRectangle(), D3DXVECTOR2(this->_dieDistance* cos(PI - i*(PI / 6.0f)) + this->_position.x, this->_dieDistance *sin(PI - i*(PI / 6.0f)) + this->_position.y), D3DXVECTOR2(1.5f, 1.5f));
				}
				else
				{
					graphics->Draw(this->_sprite.GetTexture(), this->_sprite.GetDestinationRectangle(), D3DXVECTOR2(this->_dieDistance* cos(i*(PI / 6.0f)) + this->_position.x, this->_dieDistance *sin(i*(PI / 6.0f)) + this->_position.y), D3DXVECTOR2(1.5f, 1.5f));
				}			 			
			}
			break;
		}
...
void CGutsManBullet::reset()
{
	...
	this->_sttRock = STTBOOM::FIRE;
	this->_idSprite = ID_SPRITE_GUSTMAN_ROCKFIRE;
	this->_position = D3DXVECTOR2(this->_posGutsMan.x, this->_posGutsMan.y + 150.0f);
	this->_sttRock = STTBOOM::NONE;
