...
int CCutman::Initlize()
{
	...
	this->_v = D3DXVECTOR2(-80.0f / 1000.0f, -100.0f / 1000.0f); // đơn vị pixel / tick
	...
	//bullet
	D3DXVECTOR2 _positionBullet = D3DXVECTOR2(this->_position.x, this->_position.y + this->_sprite.GetFrameHeight() / 2);
	this->_bullet = new CCutManBullet(10, ID_BULLET_CUTMAN, ResourceManager::GetSprite(ID_SPRITE_BULLET_CUT)
		, DAME_BULLET_CUTMAN, D3DXVECTOR2(0.0f / 1000.0f, 00.0f / 1000.0f), _positionBullet);
...
void CCutman::Render(CTimer* gametime, MGraphic* graphics)
{
	...
	//graphics->DrawString(str, rect, D3DCOLOR_XRGB(255, 255, 255), D3DXVECTOR2(1.0f, 1.0f), false,);
	CMoveableObject::Render(gametime, graphics);
}

void CCutman::Update(CTimer* gameTime, Megaman* rockman)
{
	...
	if (!this->_isFire && this->_spriteStatus == ID_SPRITE_CUTMAN_FIRE0 && this->_sprite.GetIndex() == 1) // không bắn thì mới lấy vị trí
	{
		...
		this->_bullet->_position = D3DXVECTOR2(this->_position.x, this->_position.y + this->_sprite.GetFrameHeight() / 2); // lấy lại vị trí của đạn								
	...
	{
									 this->_v.x = -0.09f;
									 this->_position.x += this->_v.x * gameTime->GetTime() * _speed;
	}
		break;
	case DIRECTIONACTION::RUNRIGHT:
	{
	...
	case DIRECTIONACTION::JUMPLEFT:
	{
									  this->_position.x += _v.x * gameTime->GetTime();
									  this->_position.y += _v.y * gameTime->GetTime() * _speed;
									  this->_v.y += _gravity;
									  this->_isStartJump = false;
	}
		break;
...
void CCutman::CheckStatus(CTimer* gameTime)
{
...
void CCutman::GetPositioRockMan(D3DXVECTOR2 pos)
{
