...
CEnemyMachineAuto::CEnemyMachineAuto(int id, int typeID, CSprite sprite, CSprite spriteExplodingEffect, int spriteIDBullet, ENEMYMACHINEAUTO_ORIENTATION orientation, D3DXVECTOR2 v, D3DXVECTOR2 positionBegin, int dame, int blood, int score) :CEnemy()
{
	_id = id;
	_typeID = typeID;
	_sprite = sprite;
	...
	if (_timeDelayFireRemain <= 0)
	{
		...
		for (int i = 0; i < 5; i++)
		{
			CBullet*  bullet = new CEnemyMachineAutoBullet(0, ID_BULLET_MACHINE_AUTO,
				...
				DAME_BULLET_MACHINE_AUTO, 100.f / 1000.0f, D3DXVECTOR2(posX, posY),
				angleFly + i * PI / 4.0f);
			bullet->Initlize();
