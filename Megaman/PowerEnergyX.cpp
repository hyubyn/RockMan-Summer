...
CPowerEnergyX::CPowerEnergyX(int typeID, CMoveableObject* master,CTexture textureContent, D3DXVECTOR2 positionStickOnScreen, unsigned int minValue, unsigned int  maxValue, unsigned int value, unsigned int speed) :CGameObject()
{
...
void CPowerEnergyX::Update(CTimer* gameTime)
{
	_isCompleted = false;
...
void CPowerEnergyX::UpdateCamera(Camera * camera)
{
	_position = camera->_pos + _positionStickOnScreen;
}

void CPowerEnergyX::Render(CTimer* gameTime, MGraphic* graphics)
{
	// Vẽ ảnh nền
	...
	graphics->Draw(_textureBkg.GetTexture(), destinationRect, _position, true, D3DXVECTOR2(1.0f, 1.0f), SpriteEffects::NONE);
	...
	graphics->Draw(_textureContent.GetTexture(), destinationRect, _position - D3DXVECTOR2(0, (_textureContent.GetHeight()- h) / 2), true, D3DXVECTOR2(1.0f, 1.0f), SpriteEffects::NONE);
