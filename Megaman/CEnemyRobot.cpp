...
CEnemyRobot::CEnemyRobot(int id, int typeID, CSprite sprite, CSprite spriteExplodingEffect, D3DXVECTOR2 positionBegin, int dame, int blood, int score) : CEnemy()
{
	_id = id;
	_typeID = typeID;
	_sprite = sprite;
...
#pragma region Vùng xử lý va chạm với BLOCK, ROCK objects (các đối tượng mà khi va chạm EnemyRobot phải dừng lại)
	for (std::list<CollisionInfo>::iterator item=_collidedInfoLst.begin(); item!=_collidedInfoLst.end(); ++item)
	{
		...
		case CDirection::ON_UP:
			...
			_v.y = 0 - GRAVITY*gameTime->GetTime();//Dừng chuyển động chiều y đi lên (dọc) của EnemyRobot bằng cách set v.y < 0
			break;
...
#pragma endregion
	...
	if (_state == ENEMYROBOT_STATE::STANDING)
	{
		...
		{
			_timeStandingRemain -= gameTime->GetTime();
			_sprite.SetIndex(0);
		}
	...
	{
		_sprite.SetIndex(1);
		Jump(gameTime->GetTime()*1);//Nhân 2 là để cho EnemyRobot di chuyển nhanh hơn trên quỹ đạo ném xiên
	}
...
void CEnemyRobot::Render(CTimer* gameTime, MGraphic* graphics)
{
