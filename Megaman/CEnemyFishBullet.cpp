...
CEnemyFishBullet::CEnemyFishBullet(int id, int typeID, CSprite spriteBullet, int dame, float v0, D3DXVECTOR2 beginPosition, float angleFly,int x, int y)
...
void CEnemyFishBullet::Update(CTimer* gameTime)
{
	_timeDelete += gameTime->GetTime();
	if (_timeDelete > _timeDeleteDefault)
		_state == BULLET_BASE_STATE::DIE;
	_sprite.Update(gameTime);
	UpdateBox();
}
void CEnemyFishBullet::Render(CTimer* gameTime, MGraphic* graphics)
{
	if (_timeDelete<_timeDeleteDefault)
