...
CEnemyBoom::CEnemyBoom(int id, int typeID, CSprite sprite, CSprite spriteExplodingEffect, float vy, D3DXVECTOR2 positionBegin, int dame, int blood, int score) :CEnemy()
{
	_id = id;
	_typeID = typeID;
	_sprite = sprite;
	_v = D3DXVECTOR2(0,vy);
	_vBegin = D3DXVECTOR2(0, vy);
	_dame = dame;
...
void CEnemyBoom::Update(CTimer* gameTime, Megaman* rockman)
{
	...
	if (_state == ENEMYBOOM_STATE::FLYING)
	{
		...
		if (_state == ENEMYBOOM_STATE::EXPLODING)
		{
			...
			_timeDelayAttackRemain -= gameTime->GetTime();

	this->UpdateBox();
}

void CEnemyBoom::Render(CTimer* gameTime, MGraphic* graphics)
{
	if (_state == ENEMYBOOM_STATE::FLYING)
