...
CEnemyWorkerBullet::CEnemyWorkerBullet(int id, int typeID, CSprite sprite, int dame, D3DXVECTOR2 beginPosition, float lMaxJump, float angleJump)
...
void CEnemyWorkerBullet::Update(CTimer* gameTime)
{
	...
	if (_state == BULLET_BASE_STATE::FLYING)
	{
		...
		_v.y -= GRAVITY * gameTime->GetTime() * 10;
	}
	this->UpdateBox();
}
...
void CEnemyWorkerBullet::Render(CTimer* gameTime, MGraphic* graphics)
{
	if (_state == BULLET_BASE_STATE::FLYING)
		if (_v.x<=0)
